  ...
  for (key in replacementValues) {
    replacementValues[key] =
      (typeof (replacementValues[key]) == "number")
        ? replacementValues[key].toLocaleString('es-ES', { maximumFractionDigits: 2 })
        : replacementValues[key]
  }

  // Add current date
  const nowDate = new Date().toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' })
...
function createDocumentFromTemplateOLDDD(destinationFolder, filename, templateId, docType = "doc", exportToPDF = true, copyComments = false, leftDelimiter = "{", rightDelimiter = "}", replacementValues) {

  // Escape special characters *+?()| in keys for text replacement
  for (oldKey in replacementValues) {
    const newKey = oldKey.toString()
      .replace("(", "\\\(")
      .replace(")", "\\\)")
      .replace("*", "\\\*")
      .replace("+", "\\\+")
    ...
    if (oldKey !== newKey) {
      Object.defineProperty(replacementValues, newKey,
  ...
  if (docType == "doc") {
    ...
    for (key in replacementValues) { body.replaceText(`${leftDelimiter}${key}${rightDelimiter}`, replacementValues[key]) }
    ...
    for (key in replacementValues) {
      var textFinder = excel.createTextFinder(`${leftDelimiter}${key}${rightDelimiter}`)
