...
function createDocumentFromTemplate(template) {
  ...
  const filename = `${getValue("nombre")} ${getValue("apellidos")} - ${
    template.templateName
  }`
  ...
  if (mimeType == "application/vnd.google-apps.document") {
    ...
    if (doc.getBody().findText(signatureText) != null) {
      const signature = DriveApp.getFileById(
        getValue("firmaIngeniera")
      ).getBlob()
    ...
    for (var i = 0; i < parent.getNumChildren(); i++) {
      try {
        ...
        while (range) {
          const matches = [
            ...range.getElement().asText().getText().matchAll(searchPattern),
          ].map((e) => e[0])
          matches.forEach((match) => {
            if (!valuesToBeReplaced.includes(match))
              valuesToBeReplaced.push(match)
          })
    ...
    if (
      templateId == "1p1RCsYWr8oNgr0-DnO_ky3kBp2gx30n2XLW1SzKCUeY" ||
      templateId == "1-w_4EUponNr7bUgH1Uu_vb7yZrzr4eqDiFfHFgewggU" ||
      templateId == "1EGQtUqyQOeT_bw1r89AYe2C3SyEis0mLyPPKiQZiKso" ||
      templateId == "1cvyP1Esg56g_UUQK10RGilOjGYVNO8wGRWKRhWxj1r4" ||
      templateId == "1gHNhuA3e2Te1_IKMwyo0LwAqoiz4cDfokKshoyTPOSI" ||
      templateId == "1XYiZCwf2By3XAQSQvdHH7LsK_6DTAXDvFDljurjGJxU"
    )
      createStudyAdditionalContents(doc)
  ...
  // Spreadsheets
    ...
    replacementValues.forEach((value) => {
      ...
      const namedRangeName = value
        .split(leftDelimiter)
        .pop()
        .split(rightDelimiter)[0]
      const namedRange =
        SpreadsheetApp.getActiveSpreadsheet().getRangeByName(namedRangeName)
      if (namedRange != null) {
